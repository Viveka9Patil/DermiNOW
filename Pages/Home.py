...
# Set page title
st.set_page_config(page_title="DermAIdventurers")

# Create a sidebar for navigation
page = st.sidebar.selectbox("Select a Page", ["Home", "About", "Diagnosis", "Report", "Ayurveda"])
...
if page == "Home":
    # Define user types (patient and doctor)
     ...
     st.header("Login")
    # Define a username and password for patient login
     patient_username = "patient123"
     patient_password = "pass123"

    # Define a username and password for doctor login
    ...
    # Check if login credentials are correct
     if st.button("Login"):
        if user_type == "Patient" and username == patient_username and password == patient_password:
            st.success("Logged in as a Patient: " + username)
            st.session_state.logged_in = True
            st.session_state.user_type = "Patient"
        elif user_type == "Doctor" and username == doctor_username and password == doctor_password:
...
if page == "Home":
      ...
      user_type = st.selectbox("Select User Type", user_type)
      ...
      if st.button("Login"):
        ...
        # Patient-specific content
          if page == "About":
            st.title("Welcome DermAIdventurers!")
            st.header("About Us")
        ...
        # About Us content
           ...
           # Doctor-Patient Communication Platform
            ...
            st.subheader("Biopsy Image Scanning Anakysis")
            ...
            st.subheader("Patient Monitoring (Doctor)")
            st.write("Input patient name and ID to get the patient's live health report as a doctor.")
            st.text_input("Enter Patient ID")
            patientVal = df.query('@patient == Patient ID')

            if not patientVal.empty:  
